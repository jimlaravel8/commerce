...
<v-content>
    <v-container fluid fill-height>
        <v-layout justify-center align-center wrap>
            ...
            <v-flex sm12>
                <!-- <myFilter :form="form" :role="role" style></myFilter> -->
            </v-flex>
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        ...
                        <v-flex sm2>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Roles</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm2>
                            <v-btn color="info" @click="openCreate" text>Create Role</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex sm12>
                <vue-good-table class="table-hover" :columns="columns" :rows="roles" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="isLoading" :sort-options="{enabled: true, initialSortBy: {field: 'id', type: 'asc'}}">
...
export default {
    ...
    methods: {
        openCreate() {
            eventBus.$emit("openCreateRole");

        },
        openEdit(data) {
            eventBus.$emit("openEditRole", data);
        },
        ...
        deleteItem(item) {
            this.$store.dispatch("deleteItem", "role/" + item.id).then(response => {
                this.$message({
                    type: 'success',
        ...
        openShow(data) {
            eventBus.$emit("openShowRole", data);
        },
        getRoles() {
            var payload = {
                model: 'roles',
